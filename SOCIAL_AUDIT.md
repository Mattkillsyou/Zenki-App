...
All **12 P0s** plus the Apple UGC additions (report-comments, mute, EULA, admin ban, ≤24h notify) were implemented on `feature/social-upgrade` and code-reviewed by a fresh-context adversarial agent (4 findings raised, all fixed). The proactive text content-filter was initially built but then **removed at the owner's direction** — Zenki is a small, adults-only, members-only club; UGC safety rests on report/block/mute/≤24h admin review/EULA. `npm run typecheck` and the functions build pass; the content filter has a runtime test. New data model + decisions are in [SOCIAL_CONTRACT.md](SOCIAL_CONTRACT.md); the PR carries the per-subsystem change summary, the Apple UGC checklist, and the device-only manual checklist. **Wave 2** (ranked feed, reactions, hashtags, group chats, leaderboards, notifications parity, multi-image/drafts, the remaining P1/P2s) is deferred per the approved plan. Deploy: push `firestore.rules` + `firestore.indexes.json` + functions, then immediately run the one-shot `backfillPostPrivacy` **and** `backfillFollowCounts` functions (run them right after the rules deploy so the privacy fail-open window and the count-init bug don't exist in practice). A pre-merge 4-pass adversarial re-audit (`SOCIAL_PR1_AUDIT.md`) found + fixed 2 blockers, 4 majors, and several minors that the privacy/member-guard changes had introduced.
